...
| 016 | [Capability `Arc<dyn T>` Pattern](016-capability-arc-dyn-pattern.md) | Accepted | 2026-04-23 | Canonical ownership shape for driver capability handles; dynamic dispatch at the capability boundary. |
| 017 | [Error Type Pragmatization](017-error-type-pragmatization.md) | Partial | 2026-04-23 | Keep `DaqError` enum for now; adopt MS struct pattern for new errors; `anyhow::Result` stays in capability traits. |
| 018 | [v4 Manifest-First Universal Driver UI](018-v4-manifest-first-universal-driver-ui.md) | Accepted (implementation in progress) | 2026-04-25 | Format-string variants replace the regex cliff; inline `[commands.X.ui]` synthesizes `ControlPanelConfig` so undergrads can author both protocol and UI in one TOML. |
