...
must_haves:
  truths:
    - "Device list loads from daemon and groups devices by capability"
    - "Actuator selection dropdown shows only Movable devices"
    - "Detector selection shows Readable and FrameProducer devices"
    - "1D/2D mode toggle switches visible form fields"
    - "Invalid form input shows red border and error tooltip"
  artifacts:
    - path: "crates/ui/src/panels/scan_builder.rs"
      provides: "ScanBuilderPanel struct with form state and device selection"
      min_lines: 200
    - path: "crates/ui/src/panels/mod.rs"
...
@crates/ui/src/panels/scans.rs
@crates/ui/src/panels/plan_runner.rs
@crates/ui/src/panels/devices.rs
@crates/ui/src/panels/mod.rs
@crates/ui/src/app.rs
</context>
...
Follow the `poll_async_results()` pattern from ScansPanel for handling async device refresh.
  ...
  <verify>
- File compiles: `cargo check -p ui`
...
<task type="auto">
  ...
  <files>crates/ui/src/panels/mod.rs, crates/ui/src/app.rs</files>
  <action>
**In `crates/ui/src/panels/mod.rs`:**
...
- Run GUI with `cargo run -p ui` and verify ScanBuilder tab appears
...
- `cargo clippy -p ui` passes with no warnings
  </verify>
