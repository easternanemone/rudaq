...
must_haves:
  ...
  artifacts:
    - path: "crates/ui/src/panels/scan_builder.rs"
      provides: "2D grid scan execution and visualization"
      contains: "grid_scan"
  key_links:
    - from: "crates/ui/src/panels/scan_builder.rs"
...
<task type="auto">
  ...
  <files>crates/ui/src/panels/scan_builder.rs</files>
  <action>
...
<task type="auto">
  <name>Task 3: Add completion summary panel with auto-save verification</name>
  <files>crates/ui/src/panels/scan_builder.rs</files>
  <action>
...
**Note on auto-save (external dependency):**
The DocumentWriter in storage automatically subscribes to document streams and writes to disk when enabled in daemon configuration. The daemon must be started with DocumentWriter active for auto-save to work. This is configured via:
- `config.toml`: `[storage] enabled = true, format = "hdf5"`
- Or daemon CLI: `--enable-storage`

The ScanBuilderPanel assumes the daemon has storage enabled. If storage is not configured, the "Saved to" path will be shown but the file won't exist.
...
- Verify daemon has DocumentWriter enabled: check `config.toml` has `[storage] enabled = true`
  </verify>
  <done>Completion summary panel displays scan results with save path; auto-save depends on daemon storage config</done>
</task>

<task type="checkpoint:human-verify" gate="blocking">
  <what-built>Complete form-based scan builder with 1D/2D scans, live plotting, and completion summary</what-built>
  <how-to-verify>
1. Start the daemon: `cargo run -p bin -- daemon --port 50051`
2. Start the GUI: `cargo run -p ui`
...
8. Verify auto-save:
   - After a completed scan, check that `data/{run_uid}.h5` exists
   - If file missing, verify daemon config has storage enabled
  </how-to-verify>
...
1. `cargo build -p ui` succeeds
2. `cargo clippy -p ui --all-targets` passes
...
- Data auto-saves via DocumentWriter (requires daemon storage config)
</success_criteria>
