...
must_haves:
  ...
  artifacts:
    - path: "crates/ui/src/panels/code_preview.rs"
      provides: "Syntax-highlighted code preview panel"
      exports: ["CodePreviewPanel"]
  key_links:
    - from: "crates/ui/src/panels/code_preview.rs"
      to: "crates/ui/src/graph/codegen.rs"
      via: "imports graph_to_rhai_script"
      pattern: "graph_to_rhai_script"
    - from: "crates/ui/src/panels/experiment_designer.rs"
      to: "crates/ui/src/panels/code_preview.rs"
...
<task type="auto">
  ...
  <action>
1. Add egui_code_editor to `crates/ui/Cargo.toml`:
   ```toml
...
2. Create `crates/ui/src/panels/code_preview.rs`:
   ```rust
...
3. Add to `crates/ui/src/panels/mod.rs`:
   ```rust
   mod code_preview;
  ...
  <verify>
`cargo check -p ui` compiles without errors.
  </verify>
  <done>
...
<task type="auto">
  ...
  <files>crates/ui/src/panels/experiment_designer.rs</files>
  <action>
...
7. Render code preview panel in ui() method (BEFORE main panel rendering, so it claims right side):
  ...
  <verify>
`cargo build -p ui` compiles. Run GUI and verify "Show Code" button appears in Experiment Designer toolbar.
  </verify>
  <done>
...
<task type="auto">
  ...
  <files>crates/ui/src/graph/codegen.rs</files>
  <action>
...
3. Add chrono to ui dependencies if not present (already in Cargo.toml per earlier read).

4. Update codegen tests to pass None for source_file parameter.
  </action>
  <verify>
`cargo test -p ui codegen` passes. Generated code includes timestamp and source file name.
  </verify>
  <done>
