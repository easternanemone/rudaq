...
must_haves:
  ...
  artifacts:
    - path: "crates/storage/src/zarr_writer.rs"
    ...
    - path: "crates/storage/Cargo.toml"
      ...
      contains: "zarrs"
  key_links:
    - from: "crates/storage/src/zarr_writer.rs"
      to: "zarrs crate"
...
@crates/storage/src/hdf5_writer.rs
@crates/storage/src/lib.rs
@crates/storage/Cargo.toml
</context>
...
<task type="auto">
  ...
  <files>crates/storage/Cargo.toml</files>
  <action>
...
Do NOT modify any existing features or dependencies.
  ...
  <verify>
Run: `cargo check -p storage --features storage_zarr`
...
<task type="auto">
  ...
  <files>crates/storage/src/zarr_writer.rs, crates/storage/src/lib.rs</files>
  <action>
...
Reference the zarrs crate patterns from 08-RESEARCH.md for correct API usage.
  ...
  <verify>
Run: `cargo test -p storage --features storage_zarr`
...
<task type="auto">
  ...
  <files>crates/storage/src/zarr_writer.rs</files>
  <action>
...
Use tempfile::TempDir for test isolation.
  ...
  <verify>
Run: `cargo test -p storage --features storage_zarr -- zarr`
...
2. Tests pass: `cargo test -p storage --features storage_zarr`
3. No regressions: `cargo test -p storage --features storage_hdf5`
4. Format/lint: `cargo fmt --all && cargo clippy -p storage --features storage_zarr`
</verification>
