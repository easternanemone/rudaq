...
impl<Tab> DockArea<'_, Tab> {
    ...
    pub fn show_inside(self, ui: &mut Ui, tab_viewer: &mut impl TabViewer<Tab = Tab>) {
        let _ = self.show_inside_impl(ui, tab_viewer, false);
    }

    /// Shows the docking hierarchy inside a [`Ui`] while allowing an external drag to use the
    /// dock area's native drop destination resolution.
    ///
    /// When `external_drag_active` is true, the dock area renders the same native overlay and
    /// returns the resolved [`TabDestination`] on mouse release instead of moving an existing tab.
    pub fn show_inside_with_external_drag(
        self,
        ui: &mut Ui,
        tab_viewer: &mut impl TabViewer<Tab = Tab>,
        external_drag_active: bool,
    ) -> Option<TabDestination> {
        self.show_inside_impl(ui, tab_viewer, external_drag_active)
    }
    ...
    fn show_inside_impl(
        ...
        let mut external_drop_destination = None;
                ...
                if let Some(destination) = tab_dst {
                    match state.dnd.as_ref().unwrap().drag.src {
                        TreeComponent::Tab(src_surf, src_node, src_tab) => {
                            self.dock_state
                                .move_tab((src_surf, src_node, src_tab), destination);
                        }
                        TreeComponent::External => {
                            external_drop_destination = Some(destination);
                        }
                        _ => todo!(
