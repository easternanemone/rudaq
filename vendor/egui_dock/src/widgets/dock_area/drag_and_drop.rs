...
impl TreeComponent {
    pub(super) fn as_tab_destination(&self) -> TabDestination {
        match *self {
            TreeComponent::External => unreachable!("external drags do not map directly to destinations"),
    ...
    pub(super) fn node_address(&self) -> (SurfaceIndex, Option<NodeIndex>) {
        match *self {
            TreeComponent::External => (SurfaceIndex::main(), None),
    ...
    pub(super) fn surface_address(&self) -> SurfaceIndex {
        match *self {
            TreeComponent::External => SurfaceIndex::main(),
