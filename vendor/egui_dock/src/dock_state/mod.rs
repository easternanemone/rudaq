...
impl<Tab> DockState<Tab> {
    ...
    /// Inserts a new tab at the given destination and returns its final location.
    pub fn insert_tab(
        &mut self,
        dst_tab: impl Into<TabDestination>,
        tab: Tab,
        ...
        match dst_tab.into() {
            TabDestination::Window(position) => {
                ...
                if let Some(state) = self.get_window_state_mut(surface_index) {
                    state.set_position(position.min);
                    state.set_size(position.size());
                }
                (surface_index, NodeIndex::root(), TabIndex(0))
            }
            TabDestination::Node(dst_surface, dst_node, dst_tab) => match dst_tab {
                TabInsert::Split(split) => {
                    let [_old, new] = self[dst_surface].split(dst_node, split, 0.5, Node::leaf(tab));
                    (dst_surface, new, TabIndex(0))
                }
                ...
                TabInsert::Append => {
                    ...
                    let tab_index = TabIndex(self[dst_surface][dst_node].tabs_count().saturating_sub(1));
                    (dst_surface, dst_node, tab_index)
                }
            },
            TabDestination::EmptySurface(dst_surface) => {
                assert!(self[dst_surface].is_empty());
