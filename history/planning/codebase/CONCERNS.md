...
- Files: `crates/server/src/grpc/health_service.rs`, `crates/server/src/grpc/server.rs`, `crates/server/src/grpc/scan_service.rs`, `crates/server/src/grpc/plugin_service.rs`
...
- Fix approach: Replace all `.unwrap()` calls with proper `?` operator or explicit error handling; use `into_inner()` for poisoned mutexes with recovery path (pattern already established in `crates/driver-pvcam/src/components/acquisition.rs` lines 215-224)
...
- Issue: `crates/driver-pvcam/src/components/acquisition.rs` (3802 LOC) contains complex lifecycle management for EOF callbacks, buffer modes, and frame loss detection
- Files: `crates/driver-pvcam/src/components/acquisition.rs`
...
- Files: `crates/driver-pvcam/src/components/acquisition.rs` lines 427-443
...
- Files: `crates/driver-pvcam/src/components/acquisition.rs` lines 196-227
...
- Files: `crates/ui/src/panels/image_viewer.rs` (2709 LOC), `crates/ui/src/panels/instrument_manager/mod.rs` (1743 LOC)
...
- Files: `crates/storage/src/ring_buffer.rs` and related
...
- Files: `crates/storage/src/ring_buffer.rs` (stream_id: AtomicU64 at line 75)
...
- Files: `crates/driver-pvcam/src/components/acquisition.rs` lines 44-49
...
- Files: `crates/server/src/grpc/server.rs` (server creation), config files
...
- Files: All `crates/driver-*/src/*.rs` serial port drivers
...
- Files: `crates/common/src/limits.rs` line 60, `crates/server/src/grpc/plugin_service.rs`
...
- Files: `crates/driver-pvcam/src/components/acquisition.rs` lines 446-496
...
- Files: `crates/driver-pvcam/src/components/acquisition.rs` lines 246-249 (comment references issue)
...
- Files: `crates/storage/src/ring_buffer.rs` (seqlock pattern, lines 93-98)
...
- Files: `crates/protocol/src/downsample.rs` (1463 LOC), `crates/server/src/grpc/hardware_service.rs` (uses it)
...
- Files: `crates/hardware/src/registry.rs` (async device registration)
...
- Files: `crates/hardware/src/registry.rs`, `crates/server/src/grpc/hardware_service.rs`
...
- Test coverage: `crates/server/src/grpc/error_mapping_tests.rs` covers error cases; add tests for capability queries
...
- Files: `crates/hardware/src/drivers/ell14.rs` (3181 LOC)
...
- Files: `crates/driver-comedi/src/lib.rs` (parallel epic bd-fgsx underway for validation)
...
**gRPC Health Check Mutex (server):**
...
- Migration plan: Implement wrapper trait in `hardware` to abstract serial layer; allows swapping with `serialport` crate if needed
...
- Current state: Architecture prepared but not implemented (see `crates/server/src/grpc/server.rs` auth fields)
...
- Files: `crates/server/src/grpc/health_service.rs` lines 32, 54, 79
...
- Files: `crates/driver-pvcam/src/components/acquisition.rs` lines 246-260
...
- Files: `crates/storage/src/ring_buffer.rs` (seqlock lines 93-98)
...
- Files: All `crates/driver-*/src/*.rs` serial port open paths
