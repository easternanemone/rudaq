...
@crates/scripting/src/rhai_engine.rs
</context>
...
<task type="auto">
  ...
  <action>
Create `crates/ui/src/graph/codegen.rs` with Rhai code generation:
...
5. Add `mod codegen;` to `crates/ui/src/graph/mod.rs` and re-export `graph_to_rhai_script`.
...
6. Add helper function `indent(level: usize) -> String` that returns `"  " * level`.
  ...
  <verify>
`cargo test -p ui codegen` passes. Add basic test that creates simple graph and verifies output contains expected Rhai syntax.
  </verify>
  <done>
...
8. `test_graph_to_rhai_cycle()`: Graph with cycle returns error comment
  ...
  <verify>
`cargo test -p ui codegen` passes all tests.
  </verify>
  <done>
...
grep -r "graph_to_rhai_script" crates/ui/src/graph/mod.rs
```
</verification>
