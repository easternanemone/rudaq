...
must_haves:
  ...
  artifacts:
    - path: "crates/ui/src/panels/script_editor.rs"
  ...
  key_links:
    ...
    - from: "crates/ui/src/panels/script_editor.rs"
...
3. Add helper method `set_status()` if not already present:
  ...
  <verify>
`cargo build -p ui` compiles. Run GUI, add nodes, click "Export Rhai...", verify file dialog opens with .rhai filter.
  </verify>
  <done>
...
<task type="auto">
  ...
  <action>
1. Create `crates/ui/src/panels/script_editor.rs`:
   ```rust
...
2. Add to `crates/ui/src/panels/mod.rs`:
   ```rust
   mod script_editor;
...
5. Modify ui() to show either graph editor OR script editor:
  ...
  <verify>
`cargo build -p ui` compiles. Run GUI and verify:
