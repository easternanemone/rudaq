...
If `origin` already exists, the setup script does nothing. To switch remotes:  
`bd dolt remote remove origin --force` then re-run with the desired `BEADS_DOLT_ORIGIN`.

**CLI / SQL remote parity:** `bd` expects a Dolt CLI working tree at `.beads/dolt/beads/`. The setup script runs `dolt init` there if needed so `bd dolt remote list` shows `SQL + CLI` instead of SQL-only warnings.
