...
must_haves:
  ...
  key_links:
    - from: "crates/ui/src/widgets/property_inspector.rs"
      to: "DeviceSelector"
...
4. Add device_ids parameter to PropertyInspector::show() signature:
  ...
  <verify>
    `cargo check -p ui` - Move node inspector compiles
  </verify>
  <done>
...
5. Update Scan inspector to also use DeviceSelector for actuator field
  ...
  <verify>
    `cargo check -p ui` - all inspectors compile
  </verify>
  <done>
...
5. Consider adding refresh mechanism: button in property inspector header to refetch device list
  ...
  <verify>
    `cargo build -p ui` - builds without errors
