...
must_haves:
  ...
  key_links:
    ...
    - from: "crates/ui/src/graph/validation.rs"
...
3. Ensure body nodes are NOT translated in the main topological traversal - only through loop unrolling
  ...
  <verify>
    `cargo check -p ui` - helper functions compile
  </verify>
  <done>
...
4. Update total_events calculation to account for loop iterations
  ...
  <verify>
    `cargo test -p ui` - add test for loop unrolling translation
  </verify>
  <done>
...
5. Integrate into validate_graph_structure or create separate validate_loops function called from designer panel
  ...
  <verify>
    `cargo test -p ui` - add tests for loop body validation
  </verify>
  <done>
...
<verification>
1. `cargo build -p ui` - compiles successfully
2. `cargo test -p ui` - all tests pass including new loop tests
3. Add unit tests:
