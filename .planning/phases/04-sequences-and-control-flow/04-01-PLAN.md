...
must_haves:
  ...
  artifacts:
    - path: "crates/ui/src/graph/nodes.rs"
      ...
      contains: "MoveConfig"
    - path: "crates/ui/src/widgets/device_selector.rs"
...
<task type="auto">
  ...
  <action>
1. Add egui_autocomplete to ui/Cargo.toml:
   ```toml
...
The widget wraps egui_autocomplete::AutoCompleteTextEdit with a clean API for device selection from registry.
  ...
  <verify>
    `cargo check -p ui` succeeds with no errors about missing egui_autocomplete
  </verify>
  <done>
...
Note: Scan variant unchanged (works well as-is). Add impl Default for config structs.
  ...
  <verify>
    `cargo check -p ui` passes with updated node types
  </verify>
  <done>
...
For initial compilation fix, use simpler UI that just edits the basic fields. Plan 02 will enhance with full UIs.
  ...
  <verify>
    `cargo build -p ui` succeeds with no errors
  </verify>
  <done>
...
<verification>
1. `cargo check -p ui` - no compilation errors
2. `cargo test -p ui` - existing tests pass (may need minor updates for new struct destructuring)
3. `cargo build -p ui` - full build succeeds
</verification>
