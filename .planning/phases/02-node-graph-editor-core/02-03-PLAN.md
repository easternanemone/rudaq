...
Add `pub mod property_inspector;` and export `PropertyInspector`.
  ...
  <verify>`cargo check -p ui` compiles with PropertyInspector widget</verify>
...
Adapt code to match actual API.
  ...
  <verify>`cargo check -p ui` compiles with command implementations</verify>
