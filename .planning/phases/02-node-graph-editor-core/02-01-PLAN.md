...
must_haves:
  ...
  artifacts:
    - path: "crates/ui/Cargo.toml"
      ...
      contains: "egui-snarl"
    - path: "crates/ui/src/graph/mod.rs"
  ...
  key_links:
    - from: "crates/ui/src/panels/experiment_designer.rs"
      to: "crates/ui/src/graph/viewer.rs"
      via: "snarl.show() call"
      pattern: "snarl\\.show"
    - from: "crates/ui/src/app.rs"
      to: "crates/ui/src/panels/experiment_designer.rs"
...
<task type="auto">
  ...
  <action>
Add dependencies to ui Cargo.toml:
...
Do NOT add petgraph yet - only add it when graph algorithms (cycle detection, topological sort) are actually needed.
  ...
  <verify>`cargo check -p ui` succeeds with new dependencies</verify>
...
NOTE: Consult egui-snarl 0.9 docs for exact trait method signatures - they may have changed from examples in research.
  ...
  <verify>`cargo check -p ui` compiles with graph module</verify>
...
2. **Structural verification:**
   - `crates/ui/src/graph/` directory exists with mod.rs, nodes.rs, viewer.rs
