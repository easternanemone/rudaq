...
files_modified:
  ...
  - crates/daq-core/src/experiment/mod.rs
autonomous: true
...
must_haves:
  ...
  artifacts:
    - path: "crates/daq-core/build.rs"
      provides: "Build-time git metadata capture"
      contains: "VERGEN"
    - path: "crates/daq-core/src/experiment/document.rs"
      provides: "Extended ExperimentManifest with provenance fields"
      contains: "git_commit"
  key_links:
    - from: "crates/daq-core/src/experiment/document.rs"
...
<task type="auto">
  ...
  <action>
1. Add vergen as build dependency in `crates/daq-core/Cargo.toml`:
   ```toml
   [build-dependencies]
...
2. Create or update `crates/daq-core/build.rs`:
   ```rust
...
4. If build.rs fails (not in git repo), vergen emits "VERGEN_IDEMPOTENT_OUTPUT" which we handle gracefully.
  ...
  <verify>
`cargo build -p daq-core` completes without errors. Run `cargo build -p daq-core -vv 2>&1 | grep VERGEN` to see emitted env vars.
  </verify>
  <done>
...
<task type="auto">
  ...
  <files>crates/daq-core/src/experiment/document.rs</files>
  <action>
...
4. Add sha2 to daq-core dependencies if not present:
   ```toml
   sha2 = "0.10"
   ```
...
5. Ensure new fields have `#[serde(default)]` for backwards compatibility when loading old manifests.
  ...
  <verify>
`cargo test -p daq-core document` passes. Add test that creates manifest and verifies git_commit is Some when built in git repo.
  </verify>
  <done>
...
5. Add tempfile to dev-dependencies if not present.
  ...
  <verify>
`cargo test -p daq-core document` passes all tests including new provenance tests.
  </verify>
  <done>
...
cargo build -p daq-core

# Run document tests
cargo test -p daq-core document

# Verify git env vars are set (verbose build)
cargo build -p daq-core -vv 2>&1 | grep -E "VERGEN|cargo:rustc-env"
```
</verification>
