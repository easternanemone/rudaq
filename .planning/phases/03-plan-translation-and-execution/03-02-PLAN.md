...
must_haves:
  ...
  artifacts:
    - path: "crates/ui/src/graph/execution_state.rs"
  ...
  key_links:
    ...
    - from: "crates/ui/src/graph/execution_state.rs"
      to: "EngineStatus"
      via: "status polling"
...
<task type="auto">
  ...
  <files>crates/ui/src/graph/execution_state.rs, crates/ui/src/graph/mod.rs</files>
  <action>
Create `crates/ui/src/graph/execution_state.rs`:
...
Update `crates/ui/src/graph/mod.rs`:
...
`cargo test -p ui execution_state` runs tests
  </verify>
  <done>
...
<task type="auto">
  ...
  <files>crates/ui/src/graph/viewer.rs, crates/ui/src/panels/experiment_designer.rs</files>
  <action>
Update `crates/ui/src/graph/viewer.rs` to support execution state highlighting:
...
grep -n "ExecutionState" crates/ui/src/graph/execution_state.rs

# Check Run button exists
grep -n "Run\|Pause\|Resume" crates/ui/src/panels/experiment_designer.rs
```
</verification>
