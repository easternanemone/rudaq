...
must_haves:
  ...
  artifacts:
    - path: "crates/ui/src/panels/multi_detector_grid.rs"
      provides: "Multi-detector grid layout panel"
      exports: ["MultiDetectorGrid", "DetectorPanel", "DetectorType"]
      min_lines: 120
  key_links:
    - from: "crates/ui/src/panels/multi_detector_grid.rs"
...
```
  ...
  <verify>
    ...
    - File exists at `crates/ui/src/panels/multi_detector_grid.rs`
...
2. **test_panel_management:**
  ...
  <verify>
    - `cargo test -p ui multi_detector_grid` passes all tests
  </verify>
  <done>
...
<verification>
1. `cargo check -p ui` compiles without errors
2. `cargo test -p ui multi_detector_grid` passes
