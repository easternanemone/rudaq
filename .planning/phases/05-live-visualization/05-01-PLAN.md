...
must_haves:
  ...
  artifacts:
    - path: "crates/ui/src/widgets/auto_scale_plot.rs"
      provides: "Grow-to-fit auto-scale plot wrapper"
      exports: ["AutoScalePlot", "AxisLockState"]
      min_lines: 100
  key_links:
    - from: "crates/ui/src/widgets/auto_scale_plot.rs"
      to: "egui_plot::Plot"
...
```
  ...
  <verify>
    ...
    - File exists at `crates/ui/src/widgets/auto_scale_plot.rs`
...
3. **test_reset_clears_bounds:**
  ...
  <verify>
    - `cargo test -p ui auto_scale_plot` passes all tests
  </verify>
  <done>
...
2. `cargo test -p ui auto_scale_plot` passes
