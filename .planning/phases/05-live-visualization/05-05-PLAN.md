...
Note: The frame_tx is std::sync::mpsc::SyncSender which is Sync+Send, safe to clone into async task.
  ...
  <verify>
cargo check -p ui 2>&1 | grep -E "^error" || echo "No errors"
rg "frame_tx.*try_send" crates/ui/src/panels/experiment_designer.rs
  </verify>
  <done>
...
6. Add imports for daq_proto types if not already present:
  ...
  <verify>
cargo check -p ui 2>&1 | grep -E "^error" || echo "No errors"
rg "data_tx.*try_send" crates/ui/src/panels/experiment_designer.rs
  </verify>
  <done>
...
<verification>
After both tasks:
1. `cargo check -p ui` passes with no errors
2. `rg "frame_tx.*try_send|data_tx.*try_send" crates/ui/src/panels/experiment_designer.rs` shows both senders in use
