...
Output: HDF5 annotation utilities in storage and annotation UI in RunHistoryPanel's detail view.
</objective>
...
<task type="auto">
  ...
  <files>crates/storage/src/hdf5_annotation.rs, crates/storage/src/lib.rs</files>
  <action>
Create `hdf5_annotation.rs` module in storage crate for HDF5 attribute manipulation (Pattern 2 from RESEARCH.md).
...
Add serde_json dependency if not already present (likely already exists for metadata serialization).
  ...
  <verify>
cargo build -p storage --features storage_hdf5
cargo clippy -p storage --features storage_hdf5 -- -D warnings
cargo test -p storage --features storage_hdf5 hdf5_annotation
  </verify>
...
<task type="auto">
  ...
  <files>crates/ui/src/panels/run_history.rs</files>
  <action>
...
ActionResult::SaveAnnotation(result) => {
    ...
    match result {
        Ok(()) => self.annotation_status = Some("Annotations saved ✓".to_string()),
...
Note: Since ui may not have direct dependency on storage, add to Cargo.toml:
```toml
[dependencies]
storage = { path = "../storage", optional = true, features = ["storage_hdf5"] }

[features]
default = []
storage_hdf5 = ["storage"]
```
...
- User clicks "Save Annotations" button, sees "Annotations saved ✓"
