...
enum ActionResult {
    ...
    #[cfg(feature = "storage_hdf5")]
    LoadAnnotation(Result<Option<storage::RunAnnotation>, String>),
}
    ...
    fn save_annotation(&mut self, file_path: String, runtime: &Runtime) {
        ...
        runtime.spawn(async move {
            let result = tokio::task::spawn_blocking(move || {
                ...
                let annotation = storage::RunAnnotation { notes, tags };
                storage::add_run_annotation(&path, &annotation)
            })
            .await
    ...
    fn load_annotation(&mut self, file_path: String, runtime: &Runtime) {
        ...
        runtime.spawn(async move {
            let result = tokio::task::spawn_blocking(move || {
                use std::path::PathBuf;
                let path = PathBuf::from(file_path);
                storage::read_run_annotations(&path)
            })
            .await
            .map_err(|e| e.to_string())
            .and_then(|r| r.map_err(|e| e.to_string()));
