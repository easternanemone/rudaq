...
impl ScriptEngine for PyO3Engine {
    async fn execute_script(&mut self, script: &str) -> Result<ScriptValue, ScriptError> {
        ...
        tokio::task::spawn_blocking(move || {
            let c_script =
                std::ffi::CString::new(script).map_err(|e| ScriptError::RuntimeError {
            ...
            Python::with_gil(|py| {
                ...
                let module = PyModule::from_code(py, &c_script, &c_filename, &c_module)
...
#[cfg(not(feature = "python"))]
impl PyO3Engine {
    /// Returns an error indicating python feature is required.
    ///
    /// # Errors
    ///
    /// Always returns `ScriptError::BackendError` explaining that the
    /// `python` feature must be enabled in Cargo.toml.
    ///
    /// # Example
    ///
    /// ```rust,ignore
    /// // This will fail at runtime if python is not enabled:
    /// let engine = PyO3Engine::new()?; // Error: feature not enabled
    /// ```
    pub fn new() -> Result<Self, crate::traits::ScriptError> {
        Err(crate::traits::ScriptError::BackendError {
            backend: "PyO3".to_string(),
            message: "PyO3 engine requires 'python' feature to be enabled".to_string(),
        })
    }
}
