...
use daq_core::driver::{
    ...
    DeviceComponents,
    DriverFactory as DriverFactoryTrait,
};
...
fn default_address() -> String { "0".to_string() }
...
impl GenericSerialDriverFactory {
    pub fn new(config: InstrumentConfig) -> Self {
        ...
        let capabilities = config.device.capabilities.iter().filter_map(|cap: &String| match cap.as_str() {
...
impl DriverFactoryTrait for GenericSerialDriverFactory {
    fn driver_type(&self) -> &'static str { Box::leak(self.driver_type.clone().into_boxed_str()) }
    fn name(&self) -> &'static str { Box::leak(self.name.clone().into_boxed_str()) }
    fn capabilities(&self) -> &'static [CoreCapability] { Box::leak(self.capabilities.clone().into_boxed_slice()) }
    ...
    fn build(&self, config: toml::Value) -> BoxFuture<'static, Result<DeviceComponents>> {
        ...
        Box::pin(async move {
            ...
            let baud_rate = instance.baud_rate.unwrap_or(inst_config.connection.baud_rate);
            ...
            let shared_port = {
                ...
                if let Some(p) = cache.get(&resolved_path) { p.clone() }
                else {
            ...
            Ok(DeviceComponents {
                ...
                wavelength_tunable: Some(driver_arc.clone() as Arc<dyn daq_core::capabilities::WavelengthTunable>),
                shutter_control: Some(driver_arc as Arc<dyn daq_core::capabilities::ShutterControl>),
                ..Default::default()
            })
        })
    }
    ...
    if dir.exists() {
        for entry in std::fs::read_dir(dir)? {
            ...
            if path.extension().and_then(|s| s.to_str()) == Some("toml") {
                if let Ok(f) = GenericSerialDriverFactory::from_file(&path) { factories.push(f); }
            }
        }
    }
    Ok(factories)
