...
- **Feature Flag Duplication** (bd-5bfv): Added `default-features = false` to `hardware` dependency in `server` to prevent cascading defaults
- **Circular Dependency Documentation** (bd-n80s): Documented why `driver-pvcam` only depends on `common`
...
- Added architectural notes for feature flag pass-through pattern in `server/Cargo.toml`
