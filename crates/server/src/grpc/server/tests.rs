//! Tests for the `grpc::server` parent module, covering auth validation,
//! measurement-pipeline helpers, streaming filters, script lifecycle,
//! and data-integrity annotation.
//!
//! Extracted from `server/mod.rs` (C1 step 4b) as an out-of-line child
//! module: the parent declares `#[cfg(test)] mod tests;` and the body
//! uses `super::*` to reach everything in `mod.rs` + its sibling
//! modules (re-exported as needed by the parent).
...
fn test_annotate_measurement_data_integrity_marks_gap() {
    ...
    let (source, fault) = annotate_measurement_data_integrity(&mut second, &mut last_frame_numbers)
...
fn test_annotate_measurement_data_integrity_ignores_regressions() {
    ...
    assert!(
        annotate_measurement_data_integrity(&mut measurement, &mut last_frame_numbers).is_none()
    );
