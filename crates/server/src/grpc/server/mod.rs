...
// `build_grpc_server!` macro and the `start_server` / `start_server_with_hardware`
// entry points moved to the `startup` sibling module (C1 step 3). They are re-
// exported above so `crate::grpc::server::start_server*` continues to resolve.
//
// `WebUiLayer` and friends moved to the `web_ui` sibling module (C1 step 2).
