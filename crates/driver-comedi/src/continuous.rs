...
//!
//! ```no_run
//! use driver_comedi::{ComediDevice, ContinuousStream, StreamConfig};
//!
...
struct Sink {
    #[expect(
        dead_code,
        reason = "config retained for diagnostics and future sink introspection"
    )]
    config: SinkConfig,
...
pub struct ContinuousStream {
    #[expect(
        dead_code,
        reason = "device handle retained for lifetime management; used by acquisition subsystem"
    )]
    device: ComediDevice,
    config: StreamConfig,
                                ...
                                match sink.sender.try_send(batch.clone()) {
                                    ...
                                    Err(mpsc::error::TrySendError::Full(_)) => {
                                        ...
                                        #[expect(
                                            ...
                                            reason = "batch_size * n_channels is small, fits in u64"
                                        )]
        ...
        for (_, sink) in self.sinks.read().iter() {
            ...
            if capacity > 0 {
                ...
                #[expect(
                    ...
                    reason = "channel capacity is small; precision loss is negligible for backpressure ratio"
                )]
