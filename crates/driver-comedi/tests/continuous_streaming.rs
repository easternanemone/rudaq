...
//! cargo test -p driver-comedi --test continuous_streaming --features hardware -- --nocapture --test-threads=1
//! ```
//!
//! # Test Coverage
//!
...
use driver_comedi::{ComediDevice, StreamAcquisition, StreamConfig};
use std::env;
use std::thread;
...
fn cancel_any_acquisition(device: &ComediDevice) {
    ...
    if let Some(_ai_subdev) = device.find_subdevice(driver_comedi::SubdeviceType::AnalogInput) {
