...
///
/// This function is called by `drivers::link_drivers()` to force the linker
...
/// # Usage
///
/// This function is automatically called by `drivers::link_drivers()` when
...
#[expect(
    dead_code,
    reason = "fields are read via Arc accessors in capability trait impls and drop order is load-bearing"
)]
...
impl PvcamDriver {
        ...
        let (default_port_name, default_speed_name, default_gain_name) =
            if let Some(table) = speed_table.as_ref() {
                ...
                #[expect(
                    ...
                    reason = "PVCAM speed/gain indices are non-negative i16 values"
                )]
    ...
    fn connect_params(&self) {
        ...
        // Buffer Mode (guard against streaming changes — no SDK call)
        ...
        // ROI (streaming guard only — no SDK call)
        ...
        // Binning (bd-aruo.5: streaming guard + SDK write)
        self.binning.connect_to_hardware_write({
            ...
            move |val| {
                ...
                let streaming = streaming.clone();
                Box::pin(async move {
                    tracing::debug!(param = "binning", ?val, "PVCAM hw_write called");
                    if streaming.get() {
                        tracing::debug!(
                            param = "binning",
                            "PVCAM hw_write rejected — streaming active"
                            ...
                            port.speeds.iter().find(|s| s.name == new_speed).cloned()
                    {
                        ...
                        let _ = bit_depth
                            .set_from_hardware({
                                #[expect(
                                    clippy::cast_sign_loss,
                                    reason = "PVCAM bit_depth is always positive (8, 12, 16)"
                                )]
                                {
                                    speed.bit_depth as u16
                                }
...
impl FrameProducer for PvcamDriver {
    ...
    #[expect(
        deprecated,
        reason = "implementing deprecated trait method for backward compatibility"
    )]
        ...
        match command {
            ...
            "upload_smart_stream" => {
                ...
                #[expect(
                    ...
                    reason = "SmartStream exposure values are in milliseconds, well within u32 range"
                )]
