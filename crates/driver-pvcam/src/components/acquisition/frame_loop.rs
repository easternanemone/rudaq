...
impl PvcamAcquisition {
    ...
    #[expect(
        ...
        reason = "PVCAM sequence frame loop requires all parameters for region, exposure, channels, and state tracking"
    )]
    ...
    #[expect(
        clippy::too_many_arguments,
        reason = "hardware frame loop requires all parameters for SDK handle, channels, counters, callback context, and streaming config"
    )]
