...
extern "system" fn minimal_eof_callback(p_frame_info: *const FRAME_INFO, _p_context: *mut c_void) {
    unsafe {
...
async fn test_17_minimal_sdk_callback() {
    ...
    unsafe {
        pl_get_param(
            ...
            PARAM_SER_SIZE,
            ATTR_CURRENT as i16,
            &mut ser_size as *mut _ as *mut c_void,
        );
        pl_get_param(
            hcam,
            PARAM_PAR_SIZE,
            ATTR_CURRENT as i16,
            &mut par_size as *mut _ as *mut c_void,
        );
    }
    ...
    println!(
        "[OK] Buffer allocated: {} frames, {:.2} MB",
        buffer_frames,
    ...
    while frames_acquired < TARGET_FRAMES {
        println!(
            "[MAIN LOOP {}] Waiting for EOF (timeout {}ms)...",
            frames_acquired + 1,
            TIMEOUT_MS
        );
        ...
        println!(
            "[MAIN LOOP {}] EOF received after {}ms, FrameNr={}",
            frames_acquired + 1,
            wait_elapsed,
            frame_nr
        );
        ...
        unsafe {
            if pl_exp_unlock_oldest_frame(hcam) == 0 {
                println!(
                    "[ERROR] pl_exp_unlock_oldest_frame failed: {}",
                    get_error_message()
                );
            } else {
    ...
    println!(
        "Total callbacks: {}",
        ctx.callback_count.load(Ordering::Acquire)
    );
...
extern "system" fn full_eof_callback(p_frame_info: *const FRAME_INFO, _p_context: *mut c_void) {
    ...
    let frame_nr = if !p_frame_info.is_null() {
        ...
        if info.FrameNr <= 25 || info.FrameNr % 50 == 0 {
            eprintln!(
                "[TEST18 CALLBACK] Frame {} ready, timestamp={}",
...
async fn test_21_metadata_isolation() {
        ...
        ) != 0
        {
            ...
            if md_avail != 0 {
                ...
                if pl_get_param(
                    ...
                    PARAM_METADATA_ENABLED,
                    ATTR_CURRENT,
                    &mut current_md as *mut _ as *mut _,
                ) != 0
                {
                ...
                if !use_metadata && current_md != 0 {
                    ...
                    if pl_set_param(hcam, PARAM_METADATA_ENABLED, &disable as *const _ as *mut _)
                        == 0
                    {
                        println!(
                            "  WARNING: Failed to disable metadata: {}",
                            get_error_message()
                        );
                    } else {
                    ...
                    if pl_set_param(hcam, PARAM_METADATA_ENABLED, &enable as *const _ as *mut _)
                        == 0
                    {
                        println!(
                            "  WARNING: Failed to enable metadata: {}",
                            get_error_message()
                        );
                    } else {
    ...
    if frames_acquired >= TARGET_FRAMES {
        println!(
            "RESULT: PARAM_METADATA_ENABLED manipulation is NOT the issue (200 frames achieved)"
        );
    } else {
        println!(
            "RESULT: PARAM_METADATA_ENABLED manipulation MAY be causing the issue ({} frames)",
            frames_acquired
        );
    }
    assert!(
...
async fn test_22_post_setup_circ_query() {
    ...
    unsafe {
        ...
        {
            println!(
                "  [WARN] PARAM_CIRC_BUFFER ATTR_CURRENT query failed: {}",
                get_error_message()
            );
        } else {
    ...
    if frames_acquired >= TARGET_FRAMES {
        println!(
            "RESULT: Post-setup PARAM_CIRC_BUFFER query is NOT the issue (200 frames achieved)"
        );
    } else {
        println!(
            "RESULT: Post-setup PARAM_CIRC_BUFFER query MAY be causing the issue ({} frames)",
            frames_acquired
        );
    }
    assert!(
...
async fn test_23_post_unlock_status_check() {
    ...
    println!(
        "Tests whether calling check_cont_status after each unlock causes the 19-frame cutoff.\n"
    );
        ...
        while frames_acquired < TARGET_FRAMES && streaming_clone.load(Ordering::Acquire) {
            ...
            if frames_acquired <= 25 || frames_acquired % 50 == 0 {
                unsafe {
                    ...
                    {
                        eprintln!(
                            "[POST-UNLOCK] status={}, bytes={}, cnt={}, frame={}",
                            status, bytes_arrived, buffer_cnt, frames_acquired
                        );
                    }
                }
            }
...
use driver_pvcam::components::acquisition::{
    pvcam_eof_callback, set_global_callback_ctx, CallbackContext, GLOBAL_CALLBACK_CTX,
};
        ...
        while frames < TARGET_FRAMES {
            ...
            if pending == 0 {
                ...
                eprintln!(
                    ...
                    bytes,
                    cnt,
                    callback_ctx_for_loop.pending_frames.load(Ordering::Acquire)
                );
...
use driver_pvcam::components::acquisition::clear_global_callback_ctx;
...
async fn test_27_callback_rereg_during_fallback() {
    ...
    if overwrite_start != 0 {
        ...
        println!(
            "[OK] CIRC_NO_OVERWRITE setup succeeded, frame_bytes={}",
            frame_bytes
        );
...
///
/// Tests the exact channel infrastructure used by the driver:
/// 1. tokio::sync::broadcast::channel for frame_tx
/// 2. tokio::sync::mpsc::unbounded_channel for error signaling
/// 3. std::sync::mpsc::channel for done signaling
/// 4. Storing buffer in tokio::sync::Mutex (async mutex)
/// 5. Spawning error watcher task
///
/// If this fails at ~19 frames: Issue is in channel infrastructure.
/// If this passes with 200 frames: Issue is elsewhere in driver.
#[tokio::test]
async fn test_28_full_driver_channel_infrastructure() {
    ...
    // 1. Create broadcast channel for frames (like driver's frame_tx)
    let (frame_tx, _frame_rx) = tokio::sync::broadcast::channel::<u32>(100);
    println!("[OK] Created broadcast channel for frames");

    // 2. Create tokio unbounded channel for errors (like driver's error_tx/error_rx)
    let (error_tx, mut error_rx) = tokio::sync::mpsc::unbounded_channel::<String>();
    println!("[OK] Created tokio unbounded_channel for errors");

    // 3. Create std::sync::mpsc channel for done signaling (like driver's done_tx/done_rx)
    let (done_tx, done_rx) = std::sync::mpsc::channel::<()>();
    println!("[OK] Created std::sync::mpsc::channel for done signaling");

    // 4. Create async mutex for buffer storage (like driver's circ_buffer)
    let circ_buffer_storage: Arc<tokio::sync::Mutex<Option<*mut u8>>> =
        Arc::new(tokio::sync::Mutex::new(None));
    println!("[OK] Created tokio::sync::Mutex for buffer storage");

    // 5. Create watch channel for streaming state (like driver's Parameter<bool>)
    let (streaming_tx, _streaming_rx) = tokio::sync::watch::channel(true);
    println!("[OK] Created watch channel for streaming state");

    // ========== SPAWN ERROR WATCHER TASK (like driver) ==========
    ...
    let _error_watcher = tokio::spawn(async move {
        if let Some(err) = error_rx.recv().await {
            ...
            // In driver, this sets streaming.set(false)
            let _ = streaming_for_watcher.send(false);
        }
    });
    println!("[OK] Spawned error watcher task");
    ...
    // Allocate buffer (page-aligned like driver)
    let buffer_count = 21;
    let buffer_size = frame_bytes as usize * buffer_count;
    let layout = Layout::from_size_align(buffer_size, 4096).unwrap();
    let buffer = unsafe { alloc(layout) };
    ...
    // ========== STORE BUFFER IN ASYNC MUTEX (like driver line 1806) ==========
    {
        let mut guard = circ_buffer_storage.lock().await;
        *guard = Some(buffer);
    }
    println!("[OK] Buffer stored in async mutex");
    ...
    // ========== FRAME ACQUISITION LOOP IN spawn_blocking (like driver) ==========
    println!(
    ...
    let poll_handle = tokio::task::spawn_blocking(move || {
        ...
        while *streaming_tx_clone.borrow() {
            ...
            // Send frame number through broadcast channel (like driver sending Arc<Frame>)
            ...
            // Decrement pending (like driver)
            callback_ctx_clone
                .pending_frames
                .fetch_sub(1, Ordering::AcqRel);
        }

        // Signal done (like driver)
        let _ = done_tx_clone.send(());
    });

    // Wait for poll handle (like driver)
    let _ = poll_handle.await;

    // Wait for done signal (like driver waits in Drop)
    ...
    unsafe {
        pl_exp_abort(hcam, CCS_HALT);
        // Get buffer from async mutex for dealloc
