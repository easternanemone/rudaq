...
use crate::config_v4::{InstrumentDefinition, V4Config};
...
impl InstrumentManager {
    async fn bootstrap(&mut self) {
        for definition in self.config.enabled_instruments() {
            let cloned = definition.clone();
            if !self.registry.contains_key(&cloned.id) {
                self.registry
                    .insert(cloned.id.clone(), InstrumentRecord::new(cloned.clone()));
            }
            if let Err(err) = self.start_instrument(&cloned.id).await {
                if let Some(record) = self.registry.get_mut(&cloned.id) {
                    record.status = InstrumentStatus::Failed;
                    record.last_error = Some(err.to_string());
                }
                error!(instrument_id = %cloned.id, ?err, "Failed to start instrument");
            }
        }
    }
...
impl Message<SubscribeToData> for InstrumentManager {
    ...
    async fn handle(
        ...
        self.data_publisher
            ...
            .await
            .context("failed to subscribe to data")?
    }
}
...
mod tests {
    ...
    fn test_config(instruments: Vec<InstrumentDefinition>) -> Arc<V4Config> {
        Arc::new(V4Config {
            application: crate::config_v4::ApplicationConfig {
                name: "test".to_string(),
                log_level: "info".to_string(),
            },
            actors: crate::config_v4::ActorConfig {
                default_mailbox_capacity: 100,
                spawn_timeout_ms: 1000,
                shutdown_timeout_ms: 1000,
            },
            storage: crate::config_v4::StorageConfig {
